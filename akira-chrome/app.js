...
function render(targetScenario){
  ...
  data.categories.forEach((c,ci)=>{
    ...
    c.scenarios.forEach((s,si)=>{
      ...
      const scoreCell=el('td'),input=el('input',undefined,'score');input.type='number';input.min='0';input.step='any';input.value=Number(s.score.toFixed(2));input.id='score-'+ci+'-'+si;input.style.color=colors[sr];input.setAttribute('aria-label','High score de '+s.name);
