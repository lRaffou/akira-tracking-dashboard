...
document.getElementById('refresh-tables').onclick=async()=>{const button=document.getElementById('refresh-tables');button.disabled=true;button.textContent='Actualisation…';try{await LocalTracker.scan();await refresh();document.getElementById('status').textContent='Tableaux actualisés à '+new Date().toLocaleTimeString('fr-FR')+'.';}catch(e){feedback(e);}finally{button.disabled=false;button.textContent='Actualiser les tableaux ↻';}};
