...
window.formatRunDate=function(value,part='datetime'){
 const date=value instanceof Date?value:new Date(value);
 if(!Number.isFinite(date.getTime()))return '—';
 const pad=n=>String(n).padStart(2,'0');
 const day=pad(date.getDate())+'-'+pad(date.getMonth()+1)+'-'+date.getFullYear(),time=pad(date.getHours())+'h'+pad(date.getMinutes());
 return part==='time'?time:part==='date'?day:day+' '+time;
};
...
window.LocalTracker = (() => {
  ...
  async function scan() {
    ...
    try {
      ...
      state.last_scan=formatRunDate(new Date(),'time');state.errors=errors.slice(0,5);
